...
from src.auth.schemas import UserCreate, UserRead, UserLogin, AuthToken, RefreshToken, UserChangePassword
...
async def check_refresh_token(refresh_token : RefreshToken, db : AsyncSession=Depends(get_db)):
    services = UserServices(db)
    return await services.check_user_refresh_token(refresh_token.token)
